...
## Local paths: create a directory to store the data and results
...
```{r selected_gene_barplot, fig.path="figures/schurch2016_", fig.width=8, fig.height=4, fig.cap="Barplot of the counts per sample fr a selected gene. "}
...
plotMA(res.DESeq2, colNonSig = "blue", main="MA plot")
...
write.table(stats.per.sample, file=file.path(resultDir, "stats_per_sample.tsv"),
            quote=FALSE, sep="\t", col.names =NA, row.names = TRUE)

# Export the DESeq2 result table
DESeq2.table <- file.path(resultDir, "yeast_Snf2_vs_WT_DESeq2_diff.tsv")
