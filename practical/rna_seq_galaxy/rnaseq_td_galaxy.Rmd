...
### Downloading transcript annotation (ensembl) in gtf format from UCSC table browser

```{r, echo=FALSE}
remdr$navigate("https://genome.ucsc.edu/cgi-bin/hgTables")
remdr$screenshot(file = "img/tablebrowser.png")
img <- readPNG("img/tablebrowser.png")
options(warn=0)
```

<img src="img/tablebrowser.png" width="100">
...
GTF file can be obtained both from **UCSC** genome browser or **ensembl** [ftp web site](http://www.ensembl.org/info/data/ftp/index.html). Here we will download the gtf file  from UCSC using Galaxy **UCSC Main table browser**. We will ask for ensembl annotations as numerous tables of both databases (i.e UCSC and ensembl) are synchronized.

In Galaxy select **Get Data > UCSC Main table browser**. The page that open correspond to the **Table Browser** from UCSC. Fill the form as follow:
...
- track: Ensembl Gene.
- Table: ensGene.
- In **Region** set **position** to **chr10**. 
- Select **GTF - gene transfert format** for output. Use the following string "hg19_ensembl.gtf" as **output file name**.
- Click on get output
- Click on **Send query to Galaxy**. 
- Rename the file to ensGene
- Check that the file format is in agreement with what is expected. 
- From this file select only genes located on chromosome 10 using **Filter and Sort > Filter data on any column**. 
- Rename the record to "hg19_ensembl_chr10.gtf".
...
- Select **Allow read to contribute to multiple features**.
